...
public class PasswordlessManagementClient(HttpClient http) : IPasswordlessManagementClient
{
    ...
    {
        using var response = await http.PostAsJsonAsync($"admin/apps/{appId}/magic-links/disable", new
        {
            PerformedBy = performedBy
        });
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await http.PostAsJsonAsync($"admin/apps/{applicationId}/settings", appSettings);
        response.EnsureSuccessStatusCode();
    }
}